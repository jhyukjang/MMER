...
class TANet(nn.Module):
    ...
    def forward(self, visual: torch.Tensor, audio: torch.Tensor):
        ...
        visual = visual.transpose(0, 1).contiguous()  # input.shape=[seq_len, batch, 3, 16, 112, 112]
        visual.div_(self.NORM_VALUE).sub_(self.MEAN)

        seq_len, batch, nc, snippet_duration, sample_size, _ = visual.size()
        visual = visual.view(seq_len * batch, nc, snippet_duration, sample_size, sample_size)
        with torch.no_grad():
            f_v = self.resnet(visual).squeeze()

        f_v=f_v.view(batch,seq_len,-1) # batch, seq_len, 2048 -> visual feature 
