...
class TANet(VisualStream):
    ...
    def forward(self, visual: torch.Tensor, audio: torch.Tensor):
        import pdb
        # input.shape=[batch, seq_len,  3, snippet_duration, 112, 112]


        visual.div_(self.NORM_VALUE).sub_(self.MEAN)
        batch, seq_len, nc, snippet_duration, sample_size, _ = visual.size()
        visual = visual.view(batch*seq_len, nc, snippet_duration, sample_size, sample_size)
        with torch.no_grad():
            output = self.resnet(visual).squeeze()
            
        pdb.set_trace()
        
        output=output.view(batch,seq_len,-1) # batch, seq_len, 2048
        ...
        with torch.no_grad():
            F = self.resnet(visual) # 384, 2048, 1, 4, 4



            F = torch.squeeze(F, dim=2)  # 384, 2048, 4, 4
            F = torch.flatten(F, start_dim=2) # 384, 2048, 16

        F = self.conv0(F)  # [B x 512 x 16]  ,  B = 384 = seq_len * batch
        ...
        audio = self.a_resnet(audio) # 512, 512, 1, 1
        audio = torch.flatten(audio, start_dim=1).contiguous() # 512, 512
        audio = self.a_fc(audio) # 512, 256
        audio = audio.view(self.audio_n_segments, bs, self.audio_embed_size).contiguous() # 16, 32, 256
        audio = audio.permute(1, 2, 0).contiguous() # [bs X audio_embed_size X audio_n_segments] = 32, 256, 16
